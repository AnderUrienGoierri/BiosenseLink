...
function startPythonBackendWS() {
    ...
    window.pythonWS.onerror = (e) => { logMessage('Error WS. Comprueba que server.py est� corriendo.', 'error'); };
    window.pythonWS.onclose = () => { logMessage('Desconectado del servidor Python.', 'warning'); };
}

/**
 * Lanzar visualizador de escritorio main.py desde la aplicaci�n web
 */
async function runDesktopVisualizer() {
    logMessage("Enviando orden de lanzamiento para el Visualizador Cl�nico Desktop...", "info");
    try {
        ...
        const data = await response.json();
        if (data.status === "ok") {
            logMessage("�Visualizador Cl�nico Desktop lanzado con �xito!", "success");
        ...
        logMessage(`Fallo al lanzar el Visualizador Cl�nico: ${err.message || 'servidor desconectado'}`, "error");
        if (typeof audioEngine !== 'undefined') {
            audioEngine.playHeartbeat(150);
        }
    }
